...
import app.appworks.school.stylish.databinding.ItemColorAnalysis3Binding
import app.appworks.school.stylish.databinding.ItemColorAnalysisBinding

class ColorAnalysisAdapter3(val viewModel: ColorAnalysisViewModel) : ListAdapter<Color, ColorAnalysisAdapter3.ColorViewHolder> (DiffCallback) {

    private lateinit var context: Context

    class ColorViewHolder(
        private val binding: ItemColorAnalysis3Binding,
        private val viewModel: ColorAnalysisViewModel
    ) : RecyclerView.ViewHolder(binding.root), LifecycleOwner {

        val isSelected: LiveData<Boolean> = viewModel.selectedColorPosition3.map {
            it == adapterPosition
        }
